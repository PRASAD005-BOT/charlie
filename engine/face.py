...
# Initialize MediaPipe Pose and Face Mesh modules
...
def process_hand_landmarks(rgb_frame, bgr_frame):
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            cv2.circle(bgr_frame, (x_index, y_index), 10, (255, 255, 255), -1)  # Index finger
            cv2.circle(bgr_frame, (x_middle, y_middle), 10, (0, 255, 0), -1)     # Middle finger
...
# Function to mask frame with an image at specified position and scale
def mask(frame, image, face_x, face_y, scale_factor):
    # Read the PNG image with transparency (alpha channel)
    rol = cv2.imread(image, cv2.IMREAD_UNCHANGED)  # Image with alpha channel
    rol = cv2.resize(rol, (int(300 * scale_factor), int(300 * scale_factor)), cv2.INTER_AREA)  # Resize based on scale factor
    
    frame_h, frame_w, frame_c = frame.shape  # Frame dimensions
    rol_h, rol_w, rol_c = rol.shape  # Image dimensions (with alpha channel)
    
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)  # Convert the frame to BGRA to handle transparency

    # Overlay the image on the frame, respecting transparency (alpha channel)
    for i in range(rol_h):
        for j in range(rol_w):
            if rol[i, j][3] != 0:  # Check if the alpha channel is not transparent
                if 0 <= (face_y - int(rol_h / 2)) + i < frame_h:
                    if 0 <= (face_x - int(rol_w / 2)) + j < frame_w:
                        frame[(face_y - int(rol_h / 2)) + i, (face_x - int(rol_w / 2)) + j] = rol[i, j]
    return frame

# Function to get face scale factor based on eye distance
def get_face_scale(landmarks, w, h):
    # Calculate the distance between landmarks for scaling (e.g., distance between eyes)
    ...
    # Use eye distance to calculate a scale factor for the mask (you can adjust this factor)
    scale_factor = eye_distance / 100  # Adjust divisor for better scaling
    return scale_factor
...
# Function to process hand landmarks in a separate thread
def process_hand_thread(rgb_frame, bgr_frame):
    global hand_moved, hand_zone
    process_hand_landmarks(rgb_frame, bgr_frame)
...
while cap.isOpened():
    ...
    hand_thread = threading.Thread(target=process_hand_thread, args=(rgb_frame, frame))
    hand_thread.start()

    # Process the frame for pose and face landmarks
    result_pose = pose.process(rgb_frame)
    result_face = face_mesh.process(rgb_frame)

    h, w = frame.shape[:2]

    # Process face landmarks and get the position for the face (nose as reference) first
    if result_face.multi_face_landmarks:
        for face_landmarks in result_face.multi_face_landmarks:
            # Calculate the scale factor based on face size (distance between eyes)
            scale_factor = get_face_scale(face_landmarks.landmark, w, h)
            ...
            # Apply the mask at the nose position with dynamic scaling
            frame = mask(frame, r"C:\Users\vadla\Downloads\hack.png", nose_x, nose_y, scale_factor)

    # Wait for the hand processing thread to finish
    hand_thread.join()
    ...
    # Display frame
    cv2.imshow("Gesture-based Subway Surfers with Face Mask", frame)
